...
contract CrocSwapRouter {
    ...
    function swap(
        ...
        uint128 minOut,
        uint8 reserveFlags
    ) public payable reEntrackLock returns (int128 baseFlow, int128 quoteFlow) {
        require(reserveFlags == 0, "Router does not support surplus collateral");

        preLoadTokens(base, quote, isBuy, inBaseQty, qty, minOut);
        (baseFlow, quoteFlow) = execSwap(base, quote, poolIdx, isBuy, inBaseQty, qty, tip, limitPrice, minOut);
    ...
    function preLoadTokens(address base, address quote, bool isBuy, bool inBaseQty, uint128 qty, uint128 minOut)
        private
    {
    ...
    function execSwap(
        ...
        bytes memory result = CrocSwapDex(payable(dex_)).userCmd{value: msg.value}(CrocSlots.SWAP_PROXY_IDX, swapCall);
...
contract CrocSwapRouterBypass {
    ...
    function swap(
        ...
        bytes memory swapCall =
            abi.encode(base, quote, poolIdx, isBuy, inBaseQty, qty, tip, limitPrice, minOut, reserveFlags);
        bytes memory result =
            CrocSwapDex(payable(dex_)).userCmdRouter{value: msg.value}(CrocSlots.SWAP_PROXY_IDX, swapCall, msg.sender);
