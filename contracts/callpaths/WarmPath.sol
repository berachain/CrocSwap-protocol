...
contract WarmPath is MarketSequencer, SettleLayer, ProtocolAccount {
    ...
    function commitLP(
        ...
        if (poolIdx == stableSwapPoolIdx_) {
            ...
            if (code == UserCmd.MINT_RANGE_LIQ_LP) {
                ...
                return
                    burnConcentratedLiq(base, quote, poolIdx, bidTick, askTick, liq, lpConduit, limitLower, limitHigher);
    ...
    function isBurnOrHarvest(uint8 code) internal pure returns (bool) {
        return code == UserCmd.BURN_AMBIENT_LIQ_LP || code == UserCmd.BURN_AMBIENT_BASE_LP
            || code == UserCmd.BURN_AMBIENT_QUOTE_LP || code == UserCmd.HARVEST_LP || code == UserCmd.BURN_RANGE_BASE_LP
            || code == UserCmd.BURN_RANGE_QUOTE_LP || code == UserCmd.BURN_RANGE_LIQ_LP;
    }
    ...
    function _calculateBidAskTick(address base, address quote, PoolSpecs.Pool memory pool)
        internal
        view
        returns (int24 bidTick, int24 askTick)
    {
