...
library LiquidityMath {
    ...
    /* @notice Calculates a weighted blend of adding incremental rewards mileage. */
    function calcBlend72(uint72 mileage, uint128 weight, uint128 total) private pure returns (uint72) {
        unchecked {
            // Intermediate results will always fit in 256-bits
            // Can safely cast, because result will always be smaller than original since
            // weight is less than total.
     ...
     *      delta snapshots. */
    function deltaRewardsRate72(uint72 feeMileage, uint72 oldMileage) internal pure returns (uint64) {
        ...
        if (feeMileage > oldMileage + REWARD_ROUND_DOWN) {
            ...
            // overflow.
            return mileageDelta > type(uint64).max ? type(uint64).max : uint64(mileageDelta);
        } else {
            return 0;
        }
    }
