...
contract TestSettleLayer is DepositDesk {
    ...
    function testQuerySurplus(address recv, address token) public view returns (uint128) {
        return super.querySurplus(recv, token);
    }

    function testSettleFlow(int128 flow, address token) public payable reEntrantLock {
        testSettleInner(flow, token, type(int128).max, 0, false);
    }

    function testSettleFlowTwo(int128 flowOne, int128 flowTwo, address token) public payable reEntrantLock {
        testSettleInner(flowOne, token, type(int128).max, 0, false);
        testSettleInner(flowTwo, token, type(int128).max, 0, false);
    }

    function testSettleLimit(int128 flow, address token, int128 limitQty) public payable reEntrantLock {
        testSettleInner(flow, token, limitQty, 0, false);
    }

    function testSettleDust(int128 flow, address token, uint128 dustThresh) public payable reEntrantLock {
        testSettleInner(flow, token, type(int128).max, dustThresh, false);
    }

    function testSettleReserves(int128 flow, address token) public payable reEntrantLock {
        testSettleInner(flow, token, type(int128).max, 0, true);
    }

    function testSettle(int128 flow, address token, int128 limitQty, uint128 dustThresh, bool useSurplus)
        public
        payable
        reEntrantLock
    {
    ...
    function testSettleInner(int128 flow, address token, int128 limitQty, uint128 dustThresh, bool useSurplus)
        public
        payable
    {
    ...
    function testDesposit(address owner, uint128 value, address token) internal reEntrantLock {
        depositSurplus(owner, value, token);
    }

    function testDepositTwice(address owner, uint128 valueOne, uint128 valueTwo, address token)
        internal
        reEntrantLock
    {
    ...
    function testDisburse(address owner, address recv, int128 value, address token) internal {
        lockHolder_ = owner;
